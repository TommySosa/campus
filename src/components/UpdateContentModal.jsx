...
export default function UpdateContentModal({ isOpen, onClose, id_content, handleRefresh }) {
    ...
    useEffect(() => {
        async function fetchContent() {
            try {
                const response = await axios.get(`http://localhost:4001/api/content/${id_content}`)
            ...
            <div className="relative p-4 w-full max-w-2xl max-h-full">
                <div className="relative p-4 bg-white rounded-lg shadow sm:p-5">
                    <div className="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 border-gray-600">
                        <h3 className="text-lg font-semibold text-gray-900 " id="modal-title">Editar contenido</h3>
                    ...
                    <form onSubmit={handleSubmit} aria-labelledby="modal-title">
                        ...
                        <div className="flex items-center space-x-4">
                            <button type="submit" className="text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Actualizar contenido</button>
                        </div>
                    </form>
                </div>
            </div>
