"use client"
import UpdateModal from "./UpdateDeliverableModal"
import CreateModal from "./CreateDeliverableModal"
import DeleteModal from "./DeleteDeliverableModal"
import { useEffect, useState } from "react"
import axios from "axios"
export default function DeliverableTable({ deliverables, openCreate, handleOpenCreate, handleRefresh }) {
        ...
        <>
            <table className="w-full text-left  text-gray-500 dark:text-gray-400">
                <thead className="text-gray-700 uppercase bg-gray-50 dark:text-gray-400">
                    <tr>
                        <th scope="col" className="px-4 py-4">Titulo</th>
                ...
                <tbody>
                    {
                        deliverables && deliverables.length > 0 ? deliverables.map((deliverable) => (
                            <>
                            <tr className="border-b border-gray-700" key={deliverable.id_deliverable}>
                                <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap ">{deliverable.title}</th>
                                <td className="px-4 py-3 max-w-[12rem] truncate">{deliverable.instruction}</td>
                                <td className="px-4 py-3 flex items-center justify-end">
                                    <button
                                        ...
                                        onClick={() => handleOpenUpdate(deliverable.id_deliverable)}
                                    >
                                    ...
                                    <button
                                        ...
                                        onClick={() => handleOpenDelete(deliverable.id_deliverable)}
                                    >
                        ...
                        )) : noFiltrados.length > 0 ? noFiltrados.map((deliverable) => (
                            <>
                                <tr className="border-b border-gray-700" key={deliverable.id_deliverable}>
                                    <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap ">{deliverable.title}</th>
                                    <td className="px-4 py-3 max-w-[12rem] truncate">{deliverable.instruction}</td>
                        ...
                        )) : <p className="text-center my-4">No se pudo obtener los entregables</p>
                    }
                        ...
                        selectedUpdateCourseId !== null && (
                            <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50">
                                <UpdateModal
                                    ...
                                    id_deliverable={selectedUpdateCourseId}
                        ...
                        selectedDeleteCourseId !== null && (
                            <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50">
                                <DeleteModal
                                    ...
                                    id_deliverable={selectedDeleteCourseId}
            ...
            <nav className="flex flex-col md:flex-row justify-between items-start md:items-center py-8 space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                <span className="text-sm font-normal text-gray-500 dark:text-gray-400">
                    Mostrando
                    <span className="font-semibold text-gray-900  "> {deliverables.length > 0 ? deliverables.length : noFiltrados.length} </span>
                    de
                    <span className="font-semibold text-gray-900  "> {noFiltrados ? noFiltrados.length : deliverables.length} </span>
                </span>
